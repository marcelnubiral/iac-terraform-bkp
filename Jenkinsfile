...
node {
    ...
    stage('Get services'){
        forlders = sh(script: "git log -1 --name-only --oneline | tail -n +2 | awk -F'/' '{print \$2}' | sort | uniq", returnStdout: true).trim().split('\n')
        echo "directorio ${forlders}"
        echo_all(forlders, bran)
    }
...
def echo_all(list, bn) {
    list.each { item ->
        ...
        t = sh(script: "if [ $(find ${item} -type f  | wc -l) -gt 0 ]; then echo 1; else echo 0; fi", returnStdout: true).trim()
       // t = sh(script: "if [ -d \"${item}\" ];then echo 1; else echo 0; fi", returnStdout: true).trim()
        echo "directorio ${t}"  
        if ("${t}" == "1") {
            dir("${item}") {
                template_id = sh(script: "cat values.develop.tfvars | grep awx_template_id | awk -F' = ' '{print \$2}'", returnStdout: true).trim()
                echo "TEMPLATE_ID: , ${template_id}"
                    ...
                    stage('Terraform Destroy') {
                        if (params.REQUESTED_ACTION == 'destroy') {
                            sh "set +x; terraform destroy -var 'domain_user=${domain_user}' -var 'domain_pwd=${domain_pwd}' -var 'awx_user=${awx_user}' -var 'awx_pwd=${awx_pwd}' -var 'ansible_win_user=${ansible_win_user}' -var 'ansible_win_pwd=${ansible_win_pwd}' -var-file=values.develop.tfvars -no-color --auto-approve"
                        }
                    }
                    stage('Terraform Plan'){
                        if (params.REQUESTED_ACTION != 'destroy') {
                            sh "set +x; terraform plan -var 'domain_user=${domain_user}' -var 'domain_pwd=${domain_pwd}' -var 'awx_user=${awx_user}' -var 'awx_pwd=${awx_pwd}' -var 'ansible_win_user=${ansible_win_user}' -var 'ansible_win_pwd=${ansible_win_pwd}' -var-file=values.develop.tfvars -no-color -out myplan"  
                        }
                    }
