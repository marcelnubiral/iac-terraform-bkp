            ...
            dir("${item}") {
                ...
                echo "TEMPLATE_ID: , ${template_id}"
                    ...
                    stage('Terraform Plan'){
                        if (params.REQUESTED_ACTION != 'destroy') { 
                        sh "#!/bin/bash +x terraform plan -var 'awx_user="+awx_user+"' -var 'awx_pwd="+awx_pwd+"' -var 'ansible_win_user="+ansible_win_user+"' -var 'ansible_win_pwd="+ansible_win_pwd+"' -var-file=values."+bn+".tfvars -no-color -out myplan"
                        }
                    }
